...
namespace SharpTerminal
{
	...
	public partial class RemoteDeviceUserControl : UserControl
	{
		...
		{
			var logs = mRemoteDevice.EventsLog;

			var logsCount = mRemoteDevice.EventsLog.Count;

			var listViewLogsCount = m_lvEventsLog.Items.Count;
			if (logsCount == listViewLogsCount)
				return;

			m_lvEventsLog.SuspendLayout();

			for (int i = listViewLogsCount; i < logsCount; ++i)
			{
				var log = logs[i];
