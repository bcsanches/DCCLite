...
TEST(Thinker, RunTest)
{
	...
	dcclite::broker::sys::Thinker_t ta{ "ta", ProcA};
	dcclite::broker::sys::Thinker_t tb{ "tb", ProcB };
	dcclite::broker::sys::Thinker_t tc{ "tc", ProcC };
	...
	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 10ms);
	ASSERT_EQ(g_iCounter, 0);

	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 50ms);
	ASSERT_EQ(g_iCounter, 1);

	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 110ms);
	ASSERT_EQ(g_iCounter, 2);

	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 150ms);
	ASSERT_EQ(g_iCounter, 3);

	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 300ms);
	ASSERT_EQ(g_iCounter, 3);
	...
	dcclite::broker::sys::Thinker_t::UpdateThinkers(tp + 100ms);
	ASSERT_EQ(g_iCounter, 2);	
}
...
TEST(Thinker, SameTime)
{
	...
	dcclite::broker::sys::Thinker_t taa{ "taa", ProcAA };
