...
namespace dcclite::broker::exec::dcc
{
	namespace detail
	{
		Location::Location(RName name, RName prefix, const Address beginAddress, const Address endAddress, LocationManager &owner) :
			IObject{ name },
			m_rnPrefix(prefix),
		...
		IFolderObject{name}
	{
		...
		for(auto &sectorData : sectorsData.GetArray())
		{
			...
			auto beginAddress = Address{static_cast<uint16_t>(json::GetInt(sectorData, "begin", "LocationManager::Sector"))};
			auto endAddress = Address{ static_cast<uint16_t>(json::GetInt(sectorData, "end", "LocationManager::Sector"))};
